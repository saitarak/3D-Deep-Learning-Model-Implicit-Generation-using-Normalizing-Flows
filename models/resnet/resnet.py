...
class ResNet(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, out_channels,
        ...
        self.in_norm = nn.BatchNorm3d(in_channels)
        self.double_after_norm = double_after_norm
        self.in_conv = WNConv3d(2 * in_channels, mid_channels, kernel_size, padding, bias=True)
        self.in_skip = WNConv3d(mid_channels, mid_channels, kernel_size=1, padding=0, bias=True)

        self.blocks = nn.ModuleList([ResidualBlock(mid_channels, mid_channels)
                                     for _ in range(num_blocks)])
        self.skips = nn.ModuleList([WNConv3d(mid_channels, mid_channels, kernel_size=1, padding=0, bias=True)
                                    for _ in range(num_blocks)])

        self.out_norm = nn.BatchNorm3d(mid_channels)
        self.out_conv = WNConv3d(mid_channels, out_channels, kernel_size=1, padding=0, bias=True)
