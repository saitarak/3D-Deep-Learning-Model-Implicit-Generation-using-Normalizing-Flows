...
class Generator(object):
    def __init__(self, model, exp_name, threshold = 0.1, checkpoint = None, device = torch.device("cuda")):
        ...
        self.model.eval()
        self.device = device
        self.checkpoint_path = os.path.dirname(__file__) + '/experiments/{}/checkpoints_generation/'.format( exp_name)
    ...
    def generate_point_cloud(self, data, num_steps = 10, num_points = 900000, filter_val = 0.009):
        ...
        for param in net_ndf.parameters():
        ...
        while len(samples_cpu) < num_points:
            ...
            for j in range(num_steps):
                print('refinement', j)
                df_pred = torch.clamp(train_rnvp.sample(net, net_ndf, 1, 'cuda', samples), max=self.threshold)
