...
class RealNVP(nn.Module):
    ...
    Args:
        ...
        `Coupling` layers.
    """
    def __init__(self, num_scales=2, in_channels=3, mid_channels=128, num_blocks = 3):
    ...
    def forward(self, x, reverse=True):
        ...
        if not reverse:
            ...
            x, ldj = self._pre_process(x)

        x, sldj = self.flows(x, ldj, reverse)
    ...
    def __init__(self, scale_idx, num_scales, in_channels, mid_channels, num_blocks):
        ...
        if self.is_last_block:
            ...
            self.out_couplings = nn.ModuleList([
                CouplingLayer(8 * in_channels, 4 * mid_channels, num_blocks, MaskType.CHANNEL_WISE, reverse_mask=False),
                CouplingLayer(8 * in_channels, 4 * mid_channels, num_blocks, MaskType.CHANNEL_WISE, reverse_mask=True),
                CouplingLayer(8 * in_channels, 4 * mid_channels, num_blocks, MaskType.CHANNEL_WISE, reverse_mask=False)
            ])
            self.next_block = _RealNVP(scale_idx + 1, num_scales, 4 * in_channels, 4 * mid_channels, num_blocks)
