...
def get_norm_layer(norm_type='instance'):
    if norm_type == 'batch':
        ...
        return functools.partial(nn.InstanceNorm3d, affine=False)
    else:
...
class BatchNormStats3d(nn.Module):
    """Compute BatchNorm3d normalization statistics: `mean` and `var`.
    Useful for keeping track of sum of log-determinant of Jacobians in flow models.
    Args:
        num_features (int): Number of features in the input (i.e., `C` in `(N, C, H, W, D)`).
    ...
    def __init__(self, num_features, eps=1e-5, decay=0.1):
        super(BatchNormStats3d, self).__init__()
        self.eps = eps
    ...
    def forward(self, x, training):
        ...
        used_mean = used_mean.view(1, x.size(1), 1, 1, 1).expand_as(x)
        used_var = used_var.view(1, x.size(1), 1, 1, 1).expand_as(x)
